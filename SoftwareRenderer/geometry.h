...
//向量类 DIM为向量维度, T为向量的元素类型
template <size_t DIM, typename T> struct vec {
    vec() { for (size_t i = DIM; i--; data_[i] = T()); } //所有元素设为默认值
...
template<size_t DIM, typename T> T operator*(const vec<DIM, T>& lhs, const vec<DIM, T>& rhs) {
    T ret = T();
    //不可以写为如下形式的原因在于size_t 是无符号类型, -1 会变成非常大的正数，导致循环进入死循环
    /*
    for (size_t i = DIM; i >= 0; i--) {
        ret += lhs[i] * rhs[i];
    }
    */
    for (size_t i = DIM; i--; ret += lhs[i] * rhs[i]); //i==1时,i--,i变为0,进入最后一次循环; 下轮循环时i==0,跳出
    return ret;
}
...
//将一个维度为DIM的向量 扩展到 一个维度为LEN的向量, 额外的维度填充为fill
...
//将一个维度为DIM的向量 投影到 一个维度为 LEN 的向量
